...
late <- read.csv("/Users/ricky/Desktop/statistics/Data/late.csv", header=TRUE)
# Then type:
# > View(late)
# in your R Console
```
...
Late$time <- cut(Late$Week, c(0,4,16,20),
...
Late %>%
  ggplot(aes(Week, No.ofLateChildren, col = Treatment)) + 
  geom_point() + theme_minimal()

myaov <- aov(No.ofLateChildren ~ as.factor(time) + as.factor(Treatment) + as.factor(time):as.factor(Treatment), data = Late)
...
Late %>%
  group_by(Treatment) %>%
  ggplot() +
  geom_point(aes(Treatment, No.ofLateChildren, col = Treatment)) +
  theme_minimal()
...
#Conclusion 
What we can conclude from this project is that there is a significant difference between the before group and the during and after group. The before group showed to us that most people actually picked their kids up with the occasional delay. Afterwards, more people actually left their kids and picked them up late. 
